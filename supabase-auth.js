...
function studentAssignedClasses(studentId=State.profile?.id){
  ...
  function tribecaFirstEmbeddableAttachmentV198(item={}){
    const attachments=normalizeAttachments(item);
    if(!attachments.length) return null;
    const candidates=attachments.map(att=>{
      const url=String(att?.url || att?.publicUrl || att?.public_url || att?.href || att?.path || att?.src || '').trim();
      const name=String(att?.name || att?.filename || att?.file_name || '').trim();
      const type=String(att?.type || att?.mime_type || '').toLowerCase();
      const kind=String(att?.kind || '').toLowerCase();
      return {att,url,name,type,kind};
    }).filter(x=>x.url);
    const isHtml=(x)=> /text\/html|application\/xhtml/i.test(x.type) || /\.html?(?:[?#].*)?$/i.test(x.url) || /\.html?$/i.test(x.name);
    const isPackage=(x)=> /tribeca-resource-package|resource-package|recurso-completo/i.test(x.kind) || /recurso completo/i.test(x.name);
    return candidates.find(x=>isPackage(x) && isHtml(x)) || candidates.find(isPackage) || candidates.find(isHtml) || null;
  }
  function materialEmbedValue(m={}, key='url'){
    if(key==='url'){
      const direct=String(m.embed_url || m.interactive_url || m.game_url || '').trim();
      if(direct) return direct;
      const att=tribecaFirstEmbeddableAttachmentV198(m);
      return att?.url || '';
    }
  ...
  function tribecaLooksLikeEscapedHtmlV203(value=''){
    const raw=String(value||'').trim();
    return /&lt;\s*(?:!doctype|html|head|body|style|script|main|section|div|article|iframe)\b/i.test(raw)
      && !/<\s*(?:!doctype|html|head|body|style|script|main|section|div|article|iframe)\b/i.test(raw.slice(0,1500));
  }
  function tribecaDecodeHtmlEntitiesV203(value=''){
  ...
  function normalizeEmbeddedHtml(code=''){
    ...
    if(tribecaLooksLikeEscapedHtmlV203(raw)) raw=tribecaDecodeHtmlEntitiesV203(raw);
    if(!raw) return '';
  ...
  function tribecaIsPublicPackageHtmlUrlV203(url=''){
    const raw=String(url||'').trim();
...
(function(){
  ...
  function showError(msg){
    document.body.innerHTML='<main class="fallback"><strong>No se pudo cargar el recurso.</strong><p>'+esc(msg||'Error de carga')+'</p><p><a href="'+escAttr(url)+'" target="_blank" rel="noopener">Abrir recurso en pestaña nueva</a></p></main>';
  }
...
`.replace(/<\/script/gi,'<\/script');
  ...
  function tribecaMissingEmbedNoticeV198(m={}){
    ...
    const needsEmbed=['presentation','game','video','schema','test','exam'].includes(kind);
    if(!needsEmbed) return '';
    const hasAttachments=normalizeAttachments(m).length>0;
    const teacher=roleTeacher?.();
    if(!teacher) return `<section class="material-embed-block material-embed-missing-v198"><div><strong>Recurso no disponible</strong><small>La publicación está creada, pero el recurso todavía no está preparado para verse.</small></div></section>`;
    return `<section class="material-embed-block material-embed-missing-v198"><div><strong>Recurso sin visor</strong><small>${hasAttachments?'Hay adjuntos, pero no se encontró un HTML principal embebible.':'No consta URL embebible ni código HTML guardado.'}</small></div><p>Para que se muestre dentro del aula, edita la publicación y sube el ZIP completo con <code>index.html</code> y su carpeta <code>assets</code>, o pega una URL/iframe en el campo de recurso embebido.</p></section>`;
  }
...
`;
  ...
  function materialEmbedMarkupForNewWindow(m={}){
    ...
    if(source.mode==='quiz'){
      ...
      const runner=`<!doctype html><html lang="es"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${safe(source.quiz?.title||'Test interactivo')}</title><style>
      *{box-sizing:border-box}body{margin:0;background:#fffdf7;color:#172018;font-family:Inter,system-ui,-apple-system,Segoe UI,sans-serif;line-height:1.45}.wrap{max-width:980px;margin:0 auto;padding:18px 14px 34px}.head{position:sticky;top:0;z-index:5;background:#fffdf7;border-bottom:1px solid #d7ccb5;padding:10px 0 9px}.headrow{display:flex;gap:12px;justify-content:space-between;align-items:center;flex-wrap:wrap}.head h1{font-size:1.18rem;margin:0}.meta{font-size:.86rem;color:#66706b}.bar{height:7px;background:#ebe7dc;margin-top:9px;overflow:hidden}.bar>span{display:block;height:100%;width:0;background:#0b3d22;transition:width .2s ease}.mot{margin:14px 0 10px;padding:10px 12px;border-left:4px solid #0b3d22;background:#f2f6f3;font-weight:750}.q{border:1px solid #d7ccb5;background:white;border-radius:12px;padding:14px;margin:12px 0}.q legend{font-weight:900;padding:0 5px}.q small{display:block;color:#6a665d;margin:4px 0 9px}.opts{display:grid;gap:8px}.opts label{display:flex;gap:9px;align-items:flex-start;border:1px solid #ddd8cc;border-radius:10px;padding:10px 11px;cursor:pointer}.opts input{margin-top:3px}.fb{display:none;margin-top:10px;padding:9px 10px;border-radius:8px}.q.ok{border-color:#78a98b}.q.bad{border-color:#c98b8b}.fb.ok{display:block;background:#edf7f1;color:#124e2c}.fb.bad{display:block;background:#fff1f1;color:#742020}.actions{display:flex;gap:8px;justify-content:flex-end;flex-wrap:wrap;margin-top:16px}.btn{border:1px solid #0b3d22;background:#0b3d22;color:white;border-radius:10px;padding:10px 14px;font-weight:900;cursor:pointer}.btn.secondary{background:white;color:#0b3d22}.result{margin-top:14px;padding:14px;border:1px solid #d7ccb5;border-radius:12px;background:#f7f4ec}.result strong{font-size:1.45rem}@media(max-width:600px){.wrap{padding:12px 10px 26px}.q{padding:12px}.opts label{min-height:44px}.actions .btn{flex:1;min-height:44px}}
      </style></head><body><main class="wrap"><div class="head"><div class="headrow"><div><h1 id="title">Test interactivo</h1><div class="meta" id="meta"></div></div><div class="meta" id="timer">00:00</div></div><div class="bar"><span id="barin"></span></div></div><div class="mot" id="mot">Empieza con precisión. Una pregunta cada vez 🎯</div><form id="quiz"></form><div class="actions"><button class="btn secondary" type="button" id="reset">Reiniciar</button><button class="btn" type="button" id="grade">Finalizar y corregir</button></div><div id="result"></div></main><script>
      (function(){'use strict';var raw='${encoded}',bin=atob(raw),bytes=new Uint8Array(bin.length),i;for(i=0;i<bin.length;i++)bytes[i]=bin.charCodeAt(i);var text;try{text=new TextDecoder().decode(bytes)}catch(e){text=decodeURIComponent(escape(bin))}var quiz=JSON.parse(text),form=document.getElementById('quiz'),bar=document.getElementById('barin'),meta=document.getElementById('meta'),mot=document.getElementById('mot'),res=document.getElementById('result'),started=Date.now();var good=['Bien visto 👌','Buen ritmo ⚡','Sigue así 🚀','Otra más dentro 🎯','Buen cálculo 🧠','Perfecto ✅'];var n=quiz.questions.length;document.getElementById('title').textContent=quiz.title||'Test interactivo';function safeText(v){return String(v==null?'':v)}function answered(){var c=0;quiz.questions.forEach(function(q,idx){if(form.querySelector('input[name="q'+idx+'"]:checked'))c++});return c}function update(){var a=answered();meta.textContent=a+' de '+n+' respondidas';bar.style.width=(a/n*100)+'%';mot.textContent=good[a%good.length]+' · '+a+'/'+n}quiz.questions.forEach(function(q,idx){var fs=document.createElement('fieldset');fs.className='q';fs.setAttribute('data-i',idx);var lg=document.createElement('legend');lg.textContent=(idx+1)+'. '+safeText(q.q);fs.appendChild(lg);var sm=document.createElement('small');sm.textContent='Elige la respuesta correcta';fs.appendChild(sm);var box=document.createElement('div');box.className='opts';q.opts.forEach(function(o,oi){var lab=document.createElement('label'),inp=document.createElement('input'),sp=document.createElement('span');inp.type=q.opts.filter(function(x){return x.c}).length>1?'checkbox':'radio';inp.name='q'+idx;if(inp.type==='checkbox')inp.name='q'+idx+'[]';inp.value=oi;sp.textContent=safeText(o.t);lab.appendChild(inp);lab.appendChild(sp);box.appendChild(lab);inp.addEventListener('change',update)});fs.appendChild(box);var fb=document.createElement('div');fb.className='fb';fs.appendChild(fb);form.appendChild(fs)});function grade(){var score=0;quiz.questions.forEach(function(q,idx){var fs=form.querySelector('[data-i="'+idx+'"]'),expected=[],selected=[];q.opts.forEach(function(o,oi){if(o.c)expected.push(oi)});fs.querySelectorAll('input:checked').forEach(function(x){selected.push(Number(x.value))});expected.sort();selected.sort();var ok=selected.length===expected.length&&selected.every(function(v,k){return v===expected[k]});if(ok)score++;fs.classList.remove('ok','bad');fs.classList.add(ok?'ok':'bad');var fb=fs.querySelector('.fb'),ans=expected.map(function(k){return q.opts[k].t}).join(', '),rat=q.opts.map(function(o){return o.r}).filter(Boolean)[0]||'';fb.className='fb '+(ok?'ok':'bad');fb.textContent=(ok?'Correcto ✅ ':'Revisa 🔎 ')+'Respuesta correcta: '+ans+(rat?' · '+rat:'')});var pct=Math.round(score/n*100);res.className='result';res.innerHTML='<strong>'+score+'/'+n+'</strong><p>'+pct+'% de aciertos. Revisa en verde y rojo antes de repetir.</p>';window.scrollTo(0,document.body.scrollHeight)}document.getElementById('grade').addEventListener('click',grade);document.getElementById('reset').addEventListener('click',function(){form.reset();res.innerHTML='';res.className='';form.querySelectorAll('.q').forEach(function(x){x.classList.remove('ok','bad');var f=x.querySelector('.fb');f.className='fb';f.textContent=''});started=Date.now();update();window.scrollTo(0,0)});setInterval(function(){var sec=Math.floor((Date.now()-started)/1000),m=Math.floor(sec/60),r=sec%60;document.getElementById('timer').textContent=(m<10?'0':'')+m+':'+(r<10?'0':'')+r},1000);update()})();<\/script></body></html>`;
      return `<section class="material-embed-block"><div><strong>Recurso interactivo</strong><small>Test nativo de Tribeca Aula</small></div><iframe title="Test interactivo" sandbox="allow-scripts allow-forms allow-same-origin" srcdoc="${safe(runner)}" style="min-height:960px"></iframe></section>`;
    }
    ...
    if(source.src && tribecaIsPublicPackageHtmlUrlV203(source.src)){
      return `<section class="material-embed-block"><div><strong>Recurso interactivo</strong><small>URL embebida</small></div><iframe title="Recurso interactivo" sandbox="allow-scripts allow-forms allow-same-origin allow-popups allow-downloads" srcdoc="${safe(tribecaPackageHtmlLoaderSrcdocV203(source.src, m.title||'Recurso interactivo'))}" style="min-height:${height}px"></iframe></section>`;
    }
    ...
    return `<form id="t16PublicationForm" method="post" action="javascript:void(0)" onsubmit="return window.TribecaSubmitForm ? window.TribecaSubmitForm(this,event) : false;" class="t18-publication-wizard t94-publication-wizard">
      ...
      <section class="window-panel publication-files-panel publication-files-panel-v94">
        ...
        <div class="interactive-embed-panel interactive-embed-panel-v99">
          ...
          <div class="publication-full-resource-card-v197">
            <div class="publication-full-resource-head-v197">
              <strong>Recurso completo con carpeta de assets</strong>
              <small>Para juegos, presentaciones HTML, recursos con audio, imágenes, CSS o JavaScript. Puedes subir un ZIP completo o una carpeta completa.</small>
            </div>
            <div class="publication-full-resource-grid-v197">
              <label><span>ZIP del recurso completo</span><input name="resourceZipPackage" type="file" accept=".zip,application/zip,application/x-zip-compressed"></label>
              <label><span>Carpeta completa del recurso</span><input name="resourceFolderPackage" type="file" webkitdirectory directory multiple></label>
              <label><span>HTML principal suelto</span><input name="resourceIndexFile" type="file" accept=".html,.htm,text/html"></label>
              <label><span>Carpeta de assets suelta</span><input name="resourceAssetFolder" type="file" webkitdirectory directory multiple></label>
            </div>
            <input type="hidden" name="resourcePackageJson" value="">
            <div class="publication-full-resource-preview-v197" id="resourcePackagePreview">Ningún paquete de recurso completo seleccionado.</div>
            <p class="meta">El sistema sube el HTML y todos sus assets a Supabase Storage, conserva las rutas relativas y coloca automáticamente la URL del HTML principal como recurso embebido. Es la opción correcta para juegos con audio, carpetas <code>assets</code>, CSS o JavaScript.</p>
          </div>
          <div class="window-grid">
  ...
  function attemptRowsForStudentAll(userId=''){
  ...
  function attemptRowsForStudent(userId=''){
    return attemptRowsForStudentAll(userId).filter(a=>a.hidden!==true && a.hidden!=='true' && !a.deleted_at && !a.archived_at);
  }
  function attemptVisibleForTeacher(a={}){
    return a.hidden!==true && a.hidden!=='true' && !a.deleted_at && !a.archived_at;
  }
  ...
  function activityFilterOptions(rows=[], key='subject'){
    const values=[...new Set(rows.map(a=>String((key==='subject'?(a.subject||materialById(a.material_id).subject):(a.unit_title||a.unit||materialById(a.material_id).unit_title||materialById(a.material_id).unit))||'').trim()).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'es',{numeric:true}));
    return values.map(v=>`<option value="${safe(v)}" ${State[`activity${key==='subject'?'Subject':'Unit'}Filter`]===v?'selected':''}>${safe(v)}</option>`).join('');
  }
  function attemptMatchesActivityFilters(a={}){
    const mat=materialById(a.material_id);
    const subject=String(a.subject||mat.subject||'').trim();
    const unit=String(a.unit_title||a.unit||mat.unit_title||mat.unit||'').trim();
    if(State.activitySubjectFilter && subject!==State.activitySubjectFilter) return false;
    if(State.activityUnitFilter && unit!==State.activityUnitFilter) return false;
    return true;
  }
  function activityAnalyticsContent(){
    ...
    const allVisible=(State.data.examAttempts||[]).filter(attemptVisibleForTeacher);
    const low=allVisible.filter(a=>Number(a.score||0)<=5).length;
    const repeated=[...new Set(allVisible.map(a=>`${a.user_id}:${a.material_id}`).filter(Boolean))].filter(k=>{ const [u,m]=k.split(':'); return allVisible.filter(a=>String(a.user_id)===u && String(a.material_id)===m).length>=3; }).length;
    const selectedRowsRaw=selected?attemptRowsForStudent(selected.id):[];
    const selectedRows=selectedRowsRaw.filter(attemptMatchesActivityFilters);
    return `<section class="activity-analytics-v144 activity-analytics-v145 activity-analytics-v181"><div class="window-panel clean-hero activity-hero-v144"><div><p class="eyebrow">Actividad autocorregible</p><h2>Histórico de ejercicios por alumno, materia y unidad</h2><p>Gestiona cada intento: consulta el histórico, descarga PDF, añade retroalimentación, oculta intentos antiguos o retíralos del seguimiento.</p></div><div class="overview-kpis clean-kpis"><span><strong>${allVisible.length}</strong> intentos visibles</span><span><strong>${activeStudents.length}</strong> alumnos activos</span><span><strong>${low}</strong> suficiente o menos</span><span><strong>${repeated}</strong> repeticiones</span></div></div><section class="window-panel activity-main-students-v145"><div class="section-heading"><h3>Alumnado con actividad</h3><span>${activeStudents.length}</span></div>${activityAnalyticsStudentCards(students)}</section><section class="window-panel activity-filter-panel-v181"><div class="section-heading"><h3>Filtros del histórico</h3><span>${selected?safe(displayName(selected)):''}</span></div><div class="window-grid"><label>Materia<select data-t181-activity-filter="subject"><option value="">Todas las materias</option>${activityFilterOptions(selectedRowsRaw,'subject')}</select></label><label>Unidad<select data-t181-activity-filter="unit"><option value="">Todas las unidades</option>${activityFilterOptions(selectedRowsRaw,'unit')}</select></label></div><div class="inline-actions"><button type="button" class="secondary-btn" data-t181-clear-activity-filters>Limpiar filtros</button>${selected?`<button type="button" class="secondary-btn" onclick="window.TribecaPrintTribecaDocument && window.TribecaPrintTribecaDocument('student-activity-history',{studentId:'${safe(selected.id)}'})">PDF histórico de actividad</button>`:''}</div></section><section class="window-panel activity-detail-v144 activity-detail-v145"><h3>${selected?`Detalle de ${safe(displayName(selected))}`:'Selecciona un alumno'}</h3>${selected?activityStudentAttemptsTable(selectedRows):'<div class="empty-state">Selecciona un alumno para ver su actividad.</div>'}</section></section>`;
  }
  function activityStudentAttemptsTable(rows=[]){
    if(!rows.length) return '<div class="empty-state">Este alumno no tiene intentos visibles con los filtros actuales.</div>';
    return `<div class="activity-table-wrap-v145"><table class="premium-table activity-attempt-table activity-attempt-table-v148 activity-attempt-table-v163 activity-attempt-table-v181"><thead><tr><th>Fecha</th><th>Materia</th><th>Unidad</th><th>Actividad</th><th>Nota</th><th>Intento</th><th>PDF</th><th>Retroalimentación docente</th><th>Gestión</th></tr></thead><tbody>${rows.map((a,idx)=>{ const mat=materialById(a.material_id); const same=rows.filter(x=>String(x.material_id||x.title)===String(a.material_id||a.title)); const attemptNo=same.slice().reverse().findIndex(x=>String(x.id||x.completed_at)===String(a.id||a.completed_at))+1 || (same.length-idx); const score=Number(a.score||0); const title=a.title||mat.title||'Actividad'; const ref=safe(a.id||''); const feedback=safe(a.teacher_feedback||''); const comment=safe(a.teacher_comment||''); return `<tr class="${score<=5?'is-low-score':''}"><td>${safe(fmtDT(a.completed_at||a.created_at||''))}</td><td>${attemptSubjectButton(a,mat)}</td><td>${attemptUnitButton(a,mat)}</td><td>${mat.id?`<button type="button" class="linklike activity-nav-link" data-t33-open-mat="${safe(mat.id)}">${safe(title)}</button>`:safe(title)}</td><td><strong>${score.toFixed(2)}/10</strong></td><td>${attemptNo}</td><td>${attemptPrintButton(a,'PDF')}</td><td>${ref?`<form class="attempt-feedback-form-v163" data-attempt-feedback-form="${ref}"><label>Retroalimentación<textarea name="teacherFeedback" rows="2" maxlength="1200" placeholder="Orientaciones para mejorar este intento">${feedback}</textarea></label><label>Comentario<textarea name="teacherComment" rows="2" maxlength="1200" placeholder="Comentario adicional">${comment}</textarea></label><button type="button" class="secondary-btn" data-t163-save-attempt-feedback="${ref}">Guardar</button></form>`:'Intento sin identificador'}</td><td>${ref?`<div class="attempt-manage-actions-v181"><button type="button" class="secondary-btn compact-btn" data-t181-hide-attempt="${ref}">Ocultar</button><button type="button" class="danger-btn compact-btn" data-t181-delete-attempt="${ref}">Eliminar</button></div>`:'—'}</td></tr>`; }).join('')}</tbody></table></div>`;
  }
  async function hideExamAttemptV181(attemptId){
    if(!roleTeacher()) return toast('Solo la profesora puede gestionar intentos.');
    const id=String(attemptId||'').trim(); if(!id) return;
    const {error}=await table('exam_attempts').update({hidden:true, hidden_at:new Date().toISOString(), hidden_by:State.profile.id}).eq('id',id);
    if(error) throw error;
    await loadData(true); toast('Intento ocultado del histórico visible.'); rerender();
  }
  async function deleteExamAttemptV181(attemptId){
    if(!roleTeacher()) return toast('Solo la profesora puede gestionar intentos.');
    const id=String(attemptId||'').trim(); if(!id) return;
    if(!confirm('¿Retirar este intento del histórico visible? Se conserva como registro interno, pero dejará de aparecer en Actividad del alumnado.')) return;
    const {error}=await table('exam_attempts').update({hidden:true, deleted_at:new Date().toISOString(), deleted_by:State.profile.id}).eq('id',id);
    if(error) throw error;
    await loadData(true); toast('Intento retirado del seguimiento visible.'); rerender();
  }
  ...
  function documentStudentOptionsV181(selectedId=''){
    return (State.data.students||[]).slice().sort((a,b)=>displayName(a).localeCompare(displayName(b),'es')).map(s=>`<option value="${safe(s.id)}" ${String(selectedId)===String(s.id)?'selected':''}>${safe(displayName(s))} · ${safe(academicLine(s))}</option>`).join('');
  }
  function teacherDocumentsContent(){
    ...
    return `<section class="teacher-documents-v144 teacher-documents-v144b teacher-documents-v145 teacher-documents-v181"><div class="window-panel clean-hero"><div><p class="eyebrow">Documentos rápidos</p><h2>Descargas por alumno, asistencia y pagos</h2><p>Selecciona un alumno y genera documentos útiles para ti o para las familias: recibís, asistencia mensual, histórico completo y seguimiento de actividad.</p></div></div><section class="window-panel documents-selector-v181"><div class="window-grid"><label>Alumno/a<select data-t181-doc-student>${documentStudentOptionsV181(selected?.id||'')}</select></label><label>Mes<input type="month" value="${safe(month)}" data-t181-doc-month></label></div><p class="meta">Alumno seleccionado: <strong>${selected?safe(displayName(selected)):'—'}</strong> · ${safe(monthLabel(month))}</p></section><div class="document-card-grid-v144"><article class="window-panel document-card-v144"><h3>Pagos y recibís</h3><p>Recibí mensual, histórico del alumno y resumen mensual de todo el aula.</p><div class="inline-actions">${selected?`<button type="button" class="secondary-btn" onclick="window.TribecaPrintPaymentReceipt && window.TribecaPrintPaymentReceipt('${safe(selected.id)}','${safe(month)}')">Recibí del mes</button><button type="button" class="secondary-btn" onclick="window.TribecaPrintTribecaDocument && window.TribecaPrintTribecaDocument('student-payment-month',{studentId:'${safe(selected.id)}',month:'${safe(month)}'})">Pago mensual del alumno</button><button type="button" class="secondary-btn" onclick="window.TribecaPrintTribecaDocument && window.TribecaPrintTribecaDocument('student-payment-history',{studentId:'${safe(selected.id)}'})">Histórico de pagos</button>`:''}<button type="button" class="secondary-btn" onclick="window.TribecaPrintTribecaDocument && window.TribecaPrintTribecaDocument('payments-month',{month:'${safe(month)}'})">Resumen mensual del aula</button></div></article><article class="window-panel document-card-v144"><h3>Asistencia</h3><p>Resumen mensual e histórico de asistencia del alumno, con faltas, justificaciones, pausas y asistencia presunta.</p><div class="inline-actions">${selected?`<button type="button" class="secondary-btn" onclick="window.TribecaPrintTribecaDocument && window.TribecaPrintTribecaDocument('student-attendance-month',{studentId:'${safe(selected.id)}',month:'${safe(month)}'})">Asistencia mensual</button><button type="button" class="secondary-btn" onclick="window.TribecaPrintTribecaDocument && window.TribecaPrintTribecaDocument('student-attendance-history',{studentId:'${safe(selected.id)}'})">Histórico asistencia</button>`:''}</div></article><article class="window-panel document-card-v144"><h3>Dossier para familia</h3><p>Documento combinado con pagos, asistencia y actividad autocorregible del alumno seleccionado.</p><div class="inline-actions">${selected?`<button type="button" class="primary-btn" onclick="window.TribecaPrintTribecaDocument && window.TribecaPrintTribecaDocument('student-family-dossier',{studentId:'${safe(selected.id)}',month:'${safe(month)}'})">Dossier mensual</button><button type="button" class="secondary-btn" onclick="window.TribecaPrintTribecaDocument && window.TribecaPrintTribecaDocument('student-activity-history',{studentId:'${safe(selected.id)}'})">Histórico de ejercicios</button>`:''}</div></article><article class="window-panel document-card-v144"><h3>Planificario con horas</h3><p>Descarga el planning con los horarios actuales ya cubiertos o una plantilla en blanco para cubrir a mano.</p><div class="inline-actions"><button type="button" class="secondary-btn" onclick="window.TribecaPrintTribecaDocument && window.TribecaPrintTribecaDocument('schedule',{season:'school',blank:false})">Curso escolar cubierto</button><button type="button" class="secondary-btn" onclick="window.TribecaPrintTribecaDocument && window.TribecaPrintTribecaDocument('schedule',{season:'summer',blank:false})">Verano cubierto</button><button type="button" class="secondary-btn" onclick="window.TribecaPrintTribecaDocument && window.TribecaPrintTribecaDocument('schedule',{season:'school',blank:true})">Plantilla curso</button><button type="button" class="secondary-btn" onclick="window.TribecaPrintTribecaDocument && window.TribecaPrintTribecaDocument('schedule',{season:'summer',blank:true})">Plantilla verano</button></div></article><article class="window-panel document-card-v144"><h3>Ficha persoal do alumnado</h3><p>Formulario imprimible para familias, con datos persoais, familiares, académicos y hoja médica.</p><button type="button" class="secondary-btn" onclick="window.TribecaPrintTribecaDocument && window.TribecaPrintTribecaDocument('student-form')">Descargar ficha en blanco</button></article><article class="window-panel document-card-v144 document-card-wide-v148"><h3>Instrumentos de evaluación</h3><p>Modelos propios para observar, valorar y autoevaluar sin generar datos innecesarios en la web.</p><div class="inline-actions"><button type="button" class="secondary-btn" onclick="window.TribecaPrintTribecaDocument && window.TribecaPrintTribecaDocument('checklist')">Lista de control</button><button type="button" class="secondary-btn" onclick="window.TribecaPrintTribecaDocument && window.TribecaPrintTribecaDocument('rating-scale')">Escala de valoración</button><button type="button" class="secondary-btn" onclick="window.TribecaPrintTribecaDocument && window.TribecaPrintTribecaDocument('rubric')">Rúbrica</button><button type="button" class="secondary-btn" onclick="window.TribecaPrintTribecaDocument && window.TribecaPrintTribecaDocument('evaluation-target')">Diana de evaluación</button><button type="button" class="secondary-btn" onclick="window.TribecaPrintTribecaDocument && window.TribecaPrintTribecaDocument('self-assessment')">Autoevaluación</button></div></article></div></section>`;
  }
  ...
  function monthsForStudentRecordsV181(userId=''){
    const set=new Set([State.billingMonth||defaultBillingMonth()]);
    (State.data.paymentMonths||[]).filter(p=>String(p.user_id)===String(userId)).forEach(p=>{ if(p.month) set.add(String(p.month).slice(0,7)); });
    (State.data.attendance||[]).filter(a=>String(a.user_id)===String(userId)).forEach(a=>{ if(a.class_date) set.add(String(a.class_date).slice(0,7)); });
    pauseRecords(userId).forEach(p=>{ if(p.start_date) set.add(String(p.start_date).slice(0,7)); if(p.end_date) set.add(String(p.end_date).slice(0,7)); });
    return [...set].filter(Boolean).sort().reverse();
  }
  function studentByIdV181(userId=''){
    return (State.data.students||[]).find(s=>String(s.id)===String(userId)) || null;
  }
  ...
  function studentAttendanceMonthTableV181(userId='', month=defaultBillingMonth()){
    const st=studentByIdV181(userId); if(!st) return '<p>No se encontró el alumno.</p>';
    const days=monthScheduleDays(userId,month,{includePaused:true});
    const calc=calculatePaymentAmount(userId,month);
    const rows=days.map(d=>{ const rec=attendanceRecordForScheduledDay(userId,d); const status=effectiveAttendanceStatus(userId,d); return `<tr><td>${safe(fmtDate(d.date))}</td><td>${safe(d.start)}-${safe(d.end)}</td><td>${safe(d.type==='individual'?'Individual':'Grupal')}</td><td>${status==='present'?(rec?'Asistió':'Asistencia presunta'):status==='absent'?'Falta':status==='justified'?'Justificada':'Pausa'}</td></tr>`; }).join('') || '<tr><td colspan="4">Sin horario registrado para este mes.</td></tr>';
    return `<h2>Asistencia de ${safe(displayName(st))} · ${safe(monthLabel(month))}</h2><p class="muted">Asistencias: ${safe(calc.present)} · Faltas: ${safe(calc.absent)} · Justificadas: ${safe(calc.justified)} · Pausadas: ${safe(calc.paused||0)}</p><table><thead><tr><th>Fecha</th><th>Hora</th><th>Tipo</th><th>Estado</th></tr></thead><tbody>${rows}</tbody></table>`;
  }
  function studentAttendanceHistoryDocV181(userId=''){
    return monthsForStudentRecordsV181(userId).map(m=>studentAttendanceMonthTableV181(userId,m)).join('');
  }
  function studentActivityHistoryDocV181(userId=''){
    const st=studentByIdV181(userId); if(!st) return '<p>No se encontró el alumno.</p>';
    const rows=attemptRowsForStudent(userId);
    if(!rows.length) return `<h2>Actividad autocorregible de ${safe(displayName(st))}</h2><p>Sin intentos visibles.</p>`;
    return `<h2>Actividad autocorregible de ${safe(displayName(st))}</h2><table><thead><tr><th>Fecha</th><th>Materia</th><th>Unidad</th><th>Actividad</th><th>Nota</th><th>Comentario docente</th></tr></thead><tbody>${rows.map(a=>{ const mat=materialById(a.material_id); return `<tr><td>${safe(fmtDT(a.completed_at||a.created_at||''))}</td><td>${safe(a.subject||mat.subject||'')}</td><td>${safe(a.unit_title||a.unit||mat.unit_title||mat.unit||'')}</td><td>${safe(a.title||mat.title||'Actividad')}</td><td>${Number(a.score||0).toFixed(2)}/10</td><td>${safe(a.teacher_feedback||a.teacher_comment||'—')}</td></tr>`; }).join('')}</tbody></table>`;
  }
  ...
  window.TribecaPrintTribecaDocument=function(kind, opts={}){
    ...
    const studentId=opts.studentId || State.selectedStudentId || '';
    const student=studentByIdV181(studentId);
    if(kind==='student-payment-month'){ title=`Pago mensual · ${student?displayName(student):'Alumno'}`; body=`<h1>${safe(title)}</h1>${studentPaymentMonthTableV181(studentId,month)}`; }
    else if(kind==='student-payment-history'){ title=`Histórico de pagos · ${student?displayName(student):'Alumno'}`; body=`<h1>${safe(title)}</h1>${studentPaymentHistoryDocV181(studentId)}`; }
    else if(kind==='student-attendance-month'){ title=`Asistencia mensual · ${student?displayName(student):'Alumno'}`; body=`<h1>${safe(title)}</h1>${studentAttendanceMonthTableV181(studentId,month)}`; }
    else if(kind==='student-attendance-history'){ title=`Histórico de asistencia · ${student?displayName(student):'Alumno'}`; body=`<h1>${safe(title)}</h1>${studentAttendanceHistoryDocV181(studentId)}`; }
    else if(kind==='student-activity-history'){ title=`Histórico de ejercicios · ${student?displayName(student):'Alumno'}`; body=`<h1>${safe(title)}</h1>${studentActivityHistoryDocV181(studentId)}`; }
    else if(kind==='student-family-dossier'){ title=`Dossier mensual · ${student?displayName(student):'Alumno'}`; body=`<h1>${safe(title)}</h1>${studentPaymentMonthTableV181(studentId,month)}${studentAttendanceMonthTableV181(studentId,month)}${studentActivityHistoryDocV181(studentId)}`; }
    else if(kind==='payments-month'){ title='Histórico mensual de pagos'; body=`<h1>${title}</h1><p class="muted">Mes: ${safe(monthLabel(month))}</p>${paymentSummary(month)}`; }
...
function classroomsContent(){
    ...
    return `<section class="classrooms-tool classrooms-tool-v89">
      <section class="classroom-topbar panel">
        ...
        <div class="classroom-top-actions-v181"><button type="button" class="secondary-btn classroom-base-subjects-btn" data-t92-default-all-subjects>Añadir materias base a todas</button><button type="button" class="primary-btn classroom-promote-v181" data-t181-promote-all>Promocionar curso siguiente</button></div>
      </section>
...
function classroomCard(c,i=0){
  ...
  async function promoteAllStudentsNextCourseV181(){
    if(!roleTeacher()) return toast('Solo la profesora puede promocionar alumnado.');
    if(!confirm('¿Promocionar automáticamente al alumnado al curso siguiente dentro de su mismo centro? No se sobrepromocionará al alumnado que ya esté asignado a una clase 2026/27.')) return;
    const result=await State.client.rpc('tribeca_promote_all_students_next_course_v181',{p_target_academic_year:'2026/27'});
    if(result.error) throw result.error;
    await loadData(true);
    toast('Promoción automática completada. Revisa Clases y Perfiles del alumnado.');
    rerender();
  }
  ...
  const TRIBECA_RESOURCE_BUCKET_V197 = 'tribeca-public-assets';
  function tribecaSlugV197(value=''){
    return String(value||'recurso').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-+|-+$/g,'').slice(0,72) || 'recurso';
  }
  function tribecaMimeFromNameV197(name=''){
  ...
  function tribecaCleanPackagePathV197(path=''){
    return String(path||'').replace(/\\/g,'/').split('/').map(part=>part.trim()).filter(Boolean).filter(part=>part!=='.' && part!=='..').map(part=>part.normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[^a-zA-Z0-9._-]+/g,'_')).join('/');
  }
  function tribecaCommonRootV197(paths=[]){
    const clean=paths.map(p=>String(p||'').replace(/\\/g,'/').split('/').filter(Boolean));
    if(!clean.length || clean.some(parts=>parts.length<2)) return '';
    const first=clean[0][0];
    if(!first) return '';
    return clean.every(parts=>parts[0]===first) ? first : '';
  }
  function tribecaStripRootV197(path='', root=''){
    const p=String(path||'').replace(/\\/g,'/');
    return root && (p===root || p.startsWith(root+'/')) ? p.slice(root.length).replace(/^\//,'') : p;
  }
  async function tribecaInflateRawV197(bytes){
    if(!('DecompressionStream' in window)) throw new Error('Tu navegador no permite descomprimir ZIP desde la web. Sube la carpeta completa del recurso en lugar del ZIP.');
    const stream=new Blob([bytes]).stream().pipeThrough(new DecompressionStream('deflate-raw'));
    return new Uint8Array(await new Response(stream).arrayBuffer());
  }
  function tribecaZipU16V197(view, offset){ return view.getUint16(offset, true); }
  function tribecaZipU32V197(view, offset){ return view.getUint32(offset, true); }
  function tribecaFindEocdV197(bytes){
  ...
  async function tribecaExtractZipV197(file){
    const bytes=new Uint8Array(await file.arrayBuffer());
    const view=new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
    const eocd=tribecaFindEocdV197(bytes);
    if(eocd<0) throw new Error('No se pudo leer el ZIP. Comprueba que el archivo no esté dañado.');
    const entries=tribecaZipU16V197(view, eocd+10);
    let cdOffset=tribecaZipU32V197(view, eocd+16);
    const decoder=new TextDecoder('utf-8');
    const files=[];
    for(let i=0;i<entries;i+=1){
      if(tribecaZipU32V197(view, cdOffset)!==0x02014b50) break;
      const flags=tribecaZipU16V197(view, cdOffset+8);
      const method=tribecaZipU16V197(view, cdOffset+10);
      const compressedSize=tribecaZipU32V197(view, cdOffset+20);
      const uncompressedSize=tribecaZipU32V197(view, cdOffset+24);
      const nameLen=tribecaZipU16V197(view, cdOffset+28);
      const extraLen=tribecaZipU16V197(view, cdOffset+30);
      const commentLen=tribecaZipU16V197(view, cdOffset+32);
      const localOffset=tribecaZipU32V197(view, cdOffset+42);
      const rawName=bytes.slice(cdOffset+46, cdOffset+46+nameLen);
      const name=decoder.decode(rawName);
      cdOffset += 46 + nameLen + extraLen + commentLen;
      if(!name || /\/$/.test(name)) continue;
      if(tribecaZipU32V197(view, localOffset)!==0x04034b50) continue;
      const localNameLen=tribecaZipU16V197(view, localOffset+26);
      const localExtraLen=tribecaZipU16V197(view, localOffset+28);
      const start=localOffset+30+localNameLen+localExtraLen;
      const comp=bytes.slice(start, start+compressedSize);
      let out;
      if(method===0) out=comp;
      else if(method===8) out=await tribecaInflateRawV197(comp);
      else throw new Error(`El ZIP usa un método de compresión no compatible (${method}) en ${name}.`);
      if(uncompressedSize && out.length!==uncompressedSize) console.warn('[Tribeca Aula] Tamaño ZIP inesperado', name, out.length, uncompressedSize, flags);
      files.push({path:name, blob:new Blob([out],{type:tribecaMimeFromNameV197(name)}), size:out.length, type:tribecaMimeFromNameV197(name)});
    }
    if(!files.length) throw new Error('El ZIP no contiene archivos utilizables.');
    return files;
  }
  function tribecaPackageFindIndexV197(files=[]){
    const html=files.filter(f=>/\.html?$/i.test(f.path||f.name||''));
    if(!html.length) return null;
    const exact=html.find(f=>/(^|\/)index\.html?$/i.test(f.path||f.name||''));
    if(exact) return exact;
    const top=html.find(f=>!String(f.path||f.name||'').includes('/'));
    return top || html[0];
  }
  async function tribecaUploadPublicAssetV197(path, blob, opts={}){
  ...
  async function tribecaPrepareAndUploadResourcePackageV197(form, files, sourceLabel='recurso'){
    if(!files?.length) throw new Error('No hay archivos para subir.');
    if(form) form.dataset.tribecaResourceUploading='1';
    const title=String(form?.elements?.title?.value || form?.querySelector?.('input[name="title"]')?.value || sourceLabel || 'recurso').trim();
    const rawPaths=files.map(f=>String(f.path || f.webkitRelativePath || f.name || '').replace(/\\/g,'/')).filter(Boolean);
    const root=tribecaCommonRootV197(rawPaths);
    const normalized=files.map((file,idx)=>{
      const original=String(file.path || file.webkitRelativePath || file.name || `archivo-${idx}`).replace(/\\/g,'/');
      const rel=tribecaCleanPackagePathV197(tribecaStripRootV197(original, root)) || tribecaCleanPackagePathV197(file.name || `archivo-${idx}`);
      return {file, rel};
    }).filter(x=>x.rel && !/\/$/.test(x.rel));
    const indexEntry=tribecaPackageFindIndexV197(normalized.map(x=>({path:x.rel, name:x.rel, blob:x.file.blob || x.file, type:x.file.type, size:x.file.size})));
    if(!indexEntry) throw new Error('No he encontrado un archivo HTML principal. Debe existir un index.html o un .html dentro del ZIP/carpeta.');
    const packageId=`${Date.now()}-${Math.random().toString(36).slice(2,8)}`;
    const base=`interactive-packages/${safe(State.profile?.id||'teacher')}/${packageId}-${tribecaSlugV197(title)}`;
    let indexUrl='';
    let totalBytes=0;
    for(const entry of normalized){
      const blob=entry.file.blob || entry.file;
      totalBytes += Number(blob.size || entry.file.size || 0);
      const publicUrl=await tribecaUploadPublicAssetV197(`${base}/${entry.rel}`, blob, {contentType: entry.file.type || blob.type || tribecaMimeFromNameV197(entry.rel)});
      if(entry.rel===indexEntry.path) indexUrl=publicUrl;
    }
    ...
    // creadas sin visor.
    if(form){
      form.__tribecaResourcePackageLast=pack;
    ...
    if(!['presentation','game','video','schema','exam','test'].includes(normalizeMaterialKind(currentKind))){
      const pres=form?.querySelector?.('input[name="publicationKind"][value="presentation"]');
      if(pres) pres.checked=true;
    }
    let attachments=[];
    try{ attachments=JSON.parse(form?.elements?.attachmentsJson?.value || '[]'); }catch(_e){ attachments=[]; }
    attachments=attachments.filter(a=>a?.kind!=='tribeca-resource-package-v197');
    attachments.push({kind:'tribeca-resource-package-v197', name:`Recurso completo · ${indexEntry.path}`, url:indexUrl, publicUrl:indexUrl, type:'text/html', fileCount:pack.fileCount, totalBytes:pack.totalBytes, bucket:pack.bucket, basePath:pack.basePath, indexPath:pack.indexPath});
    if(form?.elements?.attachmentsJson) form.elements.attachmentsJson.value=JSON.stringify(attachments);
    const attachmentBox=$('#attachmentPreview', form); if(attachmentBox) attachmentBox.textContent=attachments.map(f=>f.name||'Archivo').join(', ');
    const box=$('#resourcePackagePreview', form);
    if(box) box.innerHTML=`<strong>Recurso completo subido correctamente.</strong><span>${safe(indexEntry.path)} · ${normalized.length} archivo${normalized.length===1?'':'s'} · ${prettyBytes(totalBytes)}</span><a href="${safe(indexUrl)}" target="_blank" rel="noopener">Probar recurso</a>`;
    toast('Recurso completo subido. Pulsa Publicar o Guardar cambios para conservarlo en la materia.');
    if(form) form.dataset.tribecaResourceUploading='';
    return pack;
  }
  async function tribecaHandleZipPackageV197(file, form){
    const box=$('#resourcePackagePreview', form); if(box) box.textContent='Descomprimiendo ZIP y subiendo archivos…';
    const extracted=await tribecaExtractZipV197(file);
    return tribecaPrepareAndUploadResourcePackageV197(form, extracted.map(f=>({name:f.path,path:f.path,type:f.type,size:f.size,blob:f.blob})), file.name || 'zip');
  }
  async function tribecaHandleFolderPackageV197(fileList, form){
    const files=Array.from(fileList||[]).filter(f=>f && f.size>=0);
    if(!files.length) return null;
    const box=$('#resourcePackagePreview', form); if(box) box.textContent='Subiendo carpeta completa del recurso…';
    return tribecaPrepareAndUploadResourcePackageV197(form, files, 'carpeta');
  }
  async function tribecaTrySeparatePackageUploadV197(form){
    const indexFile=form.__tribecaResourceIndexFile || null;
    const assetFiles=Array.from(form.__tribecaResourceAssetFiles || []);
    if(!indexFile) return null;
    const files=[indexFile, ...assetFiles];
    const box=$('#resourcePackagePreview', form); if(box) box.textContent='Subiendo HTML principal y carpeta de assets…';
    return tribecaPrepareAndUploadResourcePackageV197(form, files, indexFile.name || 'html');
  }
  ...
  function bindGlobal() {
    ...
    document.addEventListener('click', async ev=>{
      ...
      const docMonth=ev.target.closest?.('[data-t181-doc-month]'); if(docMonth){ State.billingMonth=docMonth.value || State.billingMonth; rerender(); return; }
      const actFilter=ev.target.closest?.('[data-t181-activity-filter]'); if(actFilter){ const k=actFilter.dataset.t181ActivityFilter; if(k==='subject') State.activitySubjectFilter=actFilter.value; if(k==='unit') State.activityUnitFilter=actFilter.value; rerender(); return; }
      const clearActFilters=ev.target.closest?.('[data-t181-clear-activity-filters]'); if(clearActFilters){ ev.preventDefault(); State.activitySubjectFilter=''; State.activityUnitFilter=''; rerender(); return; }
      ...
      const promoteAllBtn=ev.target.closest?.('[data-t181-promote-all]'); if(promoteAllBtn){ ev.preventDefault(); ev.stopPropagation(); await promoteAllStudentsNextCourseV181(); return; }
